...
// export default async function MessageList({ chatId }: Props) {
//   const messages = await getMessagesAction(chatId); // 👈 same as before, rakhvu padse
...
//   return <MessageListClient chatId={chatId} messages={messages} />;
// }
import { Sparkles } from "lucide-react";
import { getMessagesAction } from "../actions/get-messages.action";
import MessageListClient from "./message-list-client";
  ...
  if (messages.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 px-4 text-center text-zinc-500">
        <div className="flex h-10 w-10 items-center justify-center rounded-full border border-zinc-800 bg-zinc-900 sm:h-12 sm:w-12">
